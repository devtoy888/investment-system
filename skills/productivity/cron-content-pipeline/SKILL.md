...
### ⚠️ WeChat Session Warmup — Add to Pre-Script to Avoid First-Push Delay

When the user doesn't chat with the bot for hours, the WeChat iLink session goes stale (`context_token` expires). The adapter has a degraded fallback (retries without token), but WeChat's server may still drop the message silently — `last_status: ok` with `last_delivery_error: null` but the user sees nothing.

**Fix:** Add a silent `getConfig` call at the top of the pre-script `main()` to refresh the iLink session before data collection. No separate heartbeat cron job needed.
...
def weixin_warmup():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as resp:
            print(f"  WeChat session warmup: {json.loads(resp.read()).get('ret', 'ok')}")
    except Exception as e:
        print(f"  WeChat warmup skipped: {e}")
```

Returns `-2` on success — session accepted/refreshed. Called before any data fetching; adds ~1s overhead.
...
- **`cronjob(action='run')` silently fails on one-shot jobs (repeat=1 / duration schedule).** When you create a one-shot cron job (e.g. `schedule="30m"` or `schedule="once at 2026-07-14T01:00:00"`) and immediately try to force-execute with `action='run'`, the job returns `execution_success: false` and gets corrupted — `name` becomes the job_id, `schedule` becomes `"?"`, `deliver` becomes `"local"`. The job is consumed from the list but never actually executes. **Fix:** do NOT use `action='run'` on one-shot jobs. Set the schedule timestamp 1-3 minutes in the future and let the scheduler pick it up. For recurring jobs (`0 0 * * *`), `action='run'` works fine. Only one-shot/interval jobs have this limitation.
...
- **Status=ok does NOT mean delivery succeeded.** A cron job can show `last_status: ok` with `last_delivery_error: null` but the user never receives the push — especially on QQ. Always verify via `session_search` that the cron session has `message_count > 1` (assistant produced output). A session with only 1 message means the LLM step failed silently. See `references/qq-cron-delivery.md`.
...
|----------|------------------------|-------|
| **WeChat** | ⚠️ May silently fail on stale session | `success=True` does NOT guarantee the user received it. If iLink's `context_token` is stale (`errcode -14`), the adapter retries without token (degraded fallback). WeChat's server may still drop the message — the cron shows `last_status: ok` with `last_delivery_error: null` but the user sees nothing. **Diagnostic:** have the user message the bot first to refresh the session, then test with `cronjob(action="run")`. After a gateway restart, the first cron push may fail but subsequent runs work once the session reconnects. |
