...
用法:
  python prune_cron_sessions.py --days 7 --dry-run   # 预览，不删除
  python prune_cron_sessions.py --days 7             # 实际删除 7 天前的 cron 日志

依赖: hermes_state（Hermes 内部 SessionDB）。SessionDB 自动读取 HERMES_HOME 环境变量
定位 state.db，无需手动传路径。
"""
import argparse
import os
...
def main():
    ...
    before = db.list_sessions_rich(source="cron", limit=100000,
