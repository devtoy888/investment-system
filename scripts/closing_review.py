...
# 备援: AKShare全量实时估算（补fundgz失效）
try:
    import akshare as ak
    df = ak.fund_value_estimation_em()
    est_col = [c for c in df.columns if '估算增长率' in c] if hasattr(df, 'columns') else None
    if est_col:
        for _, row in df.iterrows():
            ...
            if code in FUND_CODES:
                try:
                    ...
                    est_f = float(val) if val and val != '---' else 0.0
                    if code not in funds_now or funds_now[code] is None:
                        funds_now[code] = {'estimated_change': val, 'code': code, 'name': FUND_CODES.get(code,''), 'change_source': 'akshare_bulk', 'nav': ''}
                    else:
                        # AKShare实时估算覆盖所有情况（比历史数据准确）
                        old_src = funds_now[code].get('change_source', 'fundgz')
                        funds_now[code]['estimated_change'] = str(est_f)
                        funds_now[code]['change_source'] = 'akshare_bulk'
                        if old_src != 'akshare_bulk':
                            print(f"  ✅ AKShare覆盖 {FUND_CODES.get(code,'')}: {est_f:+.2f}% (替换{old_src})")
                except: pass
except Exception as e:
    print(f"  ⚠️ AKShare备援: {e}")
print(f"    ✅ 基金数据: {sum(1 for v in funds_now.values() if v)}只")
...
for code, fd in funds_now.items():
    if fd and fd.get('estimated_nav') is None:
        try:
            nav = float(fd.get('nav', 0) or 0)
            ec = float(fd.get('estimated_change', 0) or 0)
            if nav > 0:
