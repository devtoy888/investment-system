    ...
    for (const msg of messages) {
      ...
      if (!msg.key.fromMe) {
        if (WHATSAPP_MODE === 'self-chat') {
          // In self-chat mode, messages from your own number can arrive
          // as !fromMe (sent from phone, not bridge). Allow them through
          // when the sender matches the bot's own number.
          const myNumber = (sock.user?.id || '').replace(/:.*@/, '@').replace(/@.*/, '');
          const myLid = (sock.user?.lid || '').replace(/:.*@/, '@').replace(/@.*/, '');
          const senderNumber = senderId.replace(/@.*/, '');
