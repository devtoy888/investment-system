...
def get_market_overview() -> dict:
    ...
    # 1. 涨跌家数 (东财push2delay优先 → push2备胎)
    # push2实时端从Oracle ARM返回502，push2delay同套数据延迟~3秒，优先使用
    _breadth_sources_tried = []
    for em_domain in ['push2delay.eastmoney.com', 'push2.eastmoney.com']:
...
# ══════════════════════════════════════════════
# 3.6b 板块资金流（东财 push2delay clist）
# ══════════════════════════════════════════════

def get_sector_fund_flow_em() -> dict:
    """获取行业板块资金流排名（东财push2delay），返回 { 行业名: {指标} }
    
    返回示例:
    {
        '半导体': {'change_pct': 9.85, 'net_inflow_wan': 16617877504, 'rank': 5},
        '电子': {'change_pct': 7.37, 'net_inflow_wan': 27497041920, 'rank': 1},
        ...
    }
    """
    result = {}
    try:
        ...
        print(f"  📊 板块资金流(东财p2d): {success_count}个行业")
        for name, d in sorted(result.items(), key=lambda x: -x[1]['net_inflow_yi'])[:5]:
            arrow = '🔴' if d['change_pct'] > 0 else '🟢'
            print(f"    {arrow} {name}: 净流入{d['net_inflow_yi']:.1f}亿 涨跌{d['change_pct']:+.2f}%")
        track_source('sector_fund_flow', success_count > 0, f"{success_count}个行业")
    except Exception as e:
        print(f"  ⚠️ get_sector_fund_flow_em: {e}")
        track_source('sector_fund_flow', False, str(e)[:80])
    return result


def get_sector_rankings_em() -> dict:
    """获取行业板块涨跌排行（东财push2delay），返回 { 行业名: {指标} }
    
    按涨跌幅排序，不含资金流（f62/f184字段会触发额外计算，可独立调用）
    """
    result = {}
    try:
        import urllib.request, urllib.parse, json
        ...
        success_count = len(result)
        print(f"  📊 行业涨跌(东财p2d): {success_count}个行业")
        top3 = sorted(result.items(), key=lambda x: -x[1]['change_pct'])[:3]
        bot3 = sorted(result.items(), key=lambda x: x[1]['change_pct'])[:3]
        top3_str = '  '.join(f'{n}+{d["change_pct"]:+.1f}%' for n, d in top3)
        bot3_str = '  '.join(f'{n}{d["change_pct"]:+.1f}%' for n, d in bot3)
        print(f"    🔺 最强: {top3_str}")
        print(f"    🔻 最弱: {bot3_str}")
        track_source('sector_rankings', success_count > 0, f"{success_count}行业")
    except Exception as e:
        print(f"  ⚠️ get_sector_rankings_em: {e}")
        track_source('sector_rankings', False, str(e)[:80])
    return result
