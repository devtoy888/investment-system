...
\- 📦 \*\*Stable envelope\*\* — see \[SCHEMA.md\](./SCHEMA.md) for \`ok/schema\_version/data/error\`
...
xhs read "https://www.xiaohongshu.com/explore/xxx?xsec\_token=yyy" # Read by URL (uses URL token)
xhs comments 1 # Read comments for the 1st result from the last list command
xhs comments "" # View comments — paste URL to cache/reuse xsec\_token
xhs comments "" --all # Fetch ALL comments (auto-paginate all pages)
xhs comments "" --all --json # All comments as JSON
xhs comments  --xsec-token T # Use note\_id + explicit xsec\_token
...
After any listing command such as \`search\`, \`feed\`, \`hot\`, \`user-posts\`, \`favorites\`, or \`my-notes\`, the CLI stores the latest ordered note list in \`~/.xiaohongshu-cli/index\_cache.json\`.
...
├── creator\_signing.py # Creator API AES-128-CBC signature
├── constants.py # URLs, User-Agent, Chrome version, SDK config
├── exceptions.py # Structured exception hierarchy (6 error types)
├── qr\_login.py # QR code login (browser-assisted terminal QR + HTTP fallback)
├── formatter.py # Output formatting, schema envelope, Rich rendering
└── commands/
 ├── \_common.py # Shared CLI helpers (structured\_output\_options, etc.)
...
uv run pytest tests/ -v --ignore=tests/test\_integration.py -m "not smoke"

\# Smoke tests (need cookies)
uv run pytest tests/ -v -m smoke

\# Integration tests (need cookies)
uv run pytest tests/test\_integration.py -v
...
3\. Run \`xhs login\` (auto-detects browser) or \`xhs login --cookie-source \`
...
xhs read  # 阅读笔记（仅走 API）
xhs read "https://...?xsec\_token=..." # 粘贴网页 URL 直接阅读（使用 URL token）
xhs comments 1 # 查看最近一次列表里的第 1 条笔记评论
xhs comments "" # 查看评论 — 粘贴 URL 以缓存/复用 xsec\_token
xhs comments "" --all # 获取全部评论（自动翻页）
xhs comments "" --all --json # 全部评论，JSON 格式
xhs comments  --xsec-token T # 用 note\_id + 显式 xsec\_token
xhs comments  # 如果之前访问过 URL，会复用缓存 token
xhs sub-comments  # 查看评论的回复
xhs user  # 用户主页
xhs user-posts  # 用户发布的笔记
...
xhs comment  -c "好棒！" # 发评论
xhs reply 1 --comment-id X -c "谢谢" # 给最近一次列表里的第 1 条笔记回复评论
xhs reply  --comment-id X -c "谢谢" # 回复评论
xhs delete-comment  # 删除自己的评论
...
\`xhs login\` 会自动尝试所有已安装浏览器，使用第一个有有效 Cookie 的浏览器。也可用 \`--cookie-source \` 指定浏览器，或 \`--qrcode\` 使用 browser-assisted 二维码登录。其他需认证命令在 session 过期时会自动重试一次。
