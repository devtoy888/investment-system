...
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `hermes setup` or `hermes model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](https://hermes-agent.nousresearch.com/docs/user-guide/profiles) / [`configuration.md`](https://hermes-agent.nousresearch.com/docs/user-guide/configuration). If you ever see an empty string in `config.yaml`, run `hermes model` (or click **Change** in the dashboard) and Hermes will write the dict form for you.
...
### Mid-session switches and context warnings [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuring-models\#mid-session-switches-and-context-warnings "Direct link to Mid-session switches and context warnings")
...
- [Setting the main model](https://hermes-agent.nousresearch.com/docs/user-guide/configuring-models#setting-the-main-model)
  - [Mid-session switches and context warnings](https://hermes-agent.nousresearch.com/docs/user-guide/configuring-models#mid-session-switches-and-context-warnings)
