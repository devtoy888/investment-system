...
# 7. Check for script path issues — the cron runner blocks scripts that resolve outside the scripts directory
# Error looks like: "Blocked: script path resolves outside the scripts directory (...): '<script_name>'"
# Fix: readlink -f $HERMES_HOME/scripts/<script> to check if it's a symlink pointing outside
# Fix: cp --remove-destination <real_path> $HERMES_HOME/scripts/<script> to replace symlink with actual file

# 8. Only after the above 7 checks fail to identify the issue — dig into code
```
