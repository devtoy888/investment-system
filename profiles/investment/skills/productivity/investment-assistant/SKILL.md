...
│  Tencent quotes + Fund values + Weibo KOL posts + sector   │
│  analysis + fact-check → summary files to /tmp/fund_data/  │
└──────────────────────────────┬──────────────────────────────┘
                               │
                    ┌──────────▼──────────┐
                    │   Agent formats      │
...
## QQ消息推送规范（2026-07-15修订）

### 核心原则：按长度合并，不按内容主题分拆

`send_qq_bot.py` 中的 `send_markdown_in_chunks()` 函数负责拆分推送。

**旧行为（已废弃）：** 以 `═══════════════════════════════════` 分隔符切分，每个内容段独立成一条QQ消息 → 早报12+条

**新行为：** 合并所有内容为连续消息，仅按QQ 3800字符限制分段 → 早报2-3条

关键规则：
- 去掉状态行（OK / FAIL / All done / Morning cards done 等）
- 保留 `═══════════════════════════════════` 分隔线作为视觉分隔，不作为消息切分点
- 仅开头加 `## 📚 {title}` 标题头，后续段落用接续标记（📎 接上条 / 📎 续下条）
- 切分点在段落边界，不截断表格或句子

```python
# send_qq_bot.py 合并逻辑核心
lines = [l for l in content.split("\\n")
         if not l.strip().startswith(("OK ", "FAIL ", "All done", "Morning cards done", "————"))]
full = "\\n".join(lines).strip()
full_msg = f"## 📚 {title}\\n\\n{full}"
if len(full_msg) <= MAX_MSG_CHARS:  # 3800一条
    send_markdown(full_msg)
else:
    # 按段落边界切分，多段时加接续标记
    ...
```
...
早/午/收三种推送（`run_morning.py`, `run_noon.py`, `run_closing.py`）共享 `send_markdown_in_chunks()`，一处修改三份同步受益。
...
The final report is pushed to the configured home channel of the active Hermes profile.
...
| 通道 | 内容 | 格式 |
|:-----|:-----|:-----|
| **QQ消息** | 概览摘要 — 快照3-4行 + 关键操作 + AI一句话理由 + R2链接 | 纯文字+emoji，禁用Markdown标题/粗体 |
| **R2报告** | 翔实完整 — 全量数据表 + 逐基金分析 + AI深度分析 + 操作逻辑 | MD+HTML成对，HTML分区块着色渲染 |

所有报告类型（早报/午报/收盘/决策/周度）统一遵守此设计。

### 错误做法 vs 正确做法

| 错误（以前做的） | 正确（用户要求的） |
|:----------------|:-----------------|
| ❌ QQ消息只丢R2链接，无内容 | ✅ QQ消息包含快照+关键操作表 |
| ❌ R2报告塞满AI分析但操作表不清晰 | ✅ R2报告完整翔实，操作表清晰可查 |
| ❌ 14:30决策报告塞5步AI分析框架 | ✅ QQ=市场快照3行+操作表1张+AI理由100字 |

### 静默上传R2

调用 `push_report_r2.push_report()` 时会自动调用 `_output(摘要)` 打印到stdout。在wrapper脚本中必须用 `io.StringIO()` 重定向stdout：
...
try:
    md, html = push_report(report_type, title, full_content, "")
finally:
...
#### QQ Bot Setup (Per-Profile)

Each Hermes profile can have its own dedicated QQ bot. Env vars: `QQ_APP_ID` and `QQ_CLIENT_SECRET` (docs: https://hermes-agent.nousresearch.com/docs/user-guide/messaging/qqbot). Per-profile .env at `$HERMES_HOME/profiles/<name>/.env`.

**Verification:**
1. Gateway restart — `s6-svc -r /run/service/gateway-<profile>/` from a host shell (NOT from inside gateway; see `hermes-container-config` for self-kill workaround)
2. Check QQ connection: `grep "QQBot" /opt/data/profiles/<name>/logs/gateway.log`
3. Verify WebSocket: `/proc/<gw_pid>/net/tcp` → look for established connections to `api.sgroup.qq.com:443`
4. Process env may NOT show `QQ_APP_ID` — Hermes loads credentials at runtime, not from process env
5. Two .env files can coexist: `/opt/data/.env` (main) and `/opt/data/profiles/<name>/.env` (per-profile), each with different QQ credentials for different bots

**Cron delivery to QQ:** `deliver: qqbot:<OpenID>`

#### Cross-Platform `/resume` Limitation

`/resume` is intentionally scoped per platform (IDOR guard in `gateway/slash_commands.py:_resume_target_allowed`). Different `user_id` per platform (Feishu=`ou_*`, QQ=`hex_string`) means QQ `/resume` cannot see Feishu sessions. This is by design.

**Workaround:** ask the agent directly from any platform — "帮我找飞书上'xxx'那个会话" — agent uses `session_search` which searches all sessions across all platforms.
...
- `references/kol-deep-analysis.md` — 2026-07-14 KOL深度分析引擎：5级80+黑话字典、赛道方向分类、`generate_kol_deep_analysis()`、英文RSS标题翻译、结构化卡片模式。
- `references/kol-following-analysis.md` — 唐史主任's following list analysis (negative finding).
- `references/kol-daily-analysis-rules.md` — 📌 2026-07-15 日报KOL采集规则：今日/历史分离、选择性解读、事实核查、去互动数。
- `references/data-source-fallback.md` — 📌 2026-07-15 数据源降级策略：快照→存档→实时API(⚠️盘中)。
- `references/qq-message-merge.md` — 📌 2026-07-15 QQ合并推送：按3800字符上限合并，不按内容主题拆分。
...
| `references/feishu-message-format.md` | 2026-07-13 飞书消息格式优化 |
| `references/feishu-card-optimization.md` | 2026-07-13 飞书Card 2.0组件优化分析 |
...
2026-07-06 用户要求对4支弱势持仓基金自动化加仓提醒。no_agent cron静默监控。
...
### Pre-Script + Agent Cron Pattern (Updated 2026-07-15: 13 cron jobs, no_agent mode)

The cron uses **`no_agent=True`** with a **wrapper script** that handles ALL delivery:

1. **Script runs** (the cron's `script` field):
   - Calls the data collection pre-script
   - Calls the card-sending script (e.g. `send_morning_cards.py`) via direct Feishu API
   - **stdout is suppressed** (redirected to `io.StringIO()`) so cron delivery sends nothing

2. **Cards go to user directly** (sent by script via Feishu API):
   - Each logical section = one Card 2.0 card with native `table` components
   - Cards bypass the adapter entirely — no sitecustomize pipeline involvement
   - The user sees ONLY cards, no LLM-generated text messages

3. **Why `no_agent=True`?** The LLM agent's response is delivered through the Feishu adapter's `send()` method, but `sitecustomize.py` patching is NOT active in the cron agent's Python process. This means LLM text responses are sent as plain text — never converted to cards. Using `no_agent=True` eliminates this problem.

**Critical: wrapper scripts must suppress stdout** — With `no_agent=True`, the script's stdout is delivered verbatim to the user. The raw log output from subprocess calls (`✅ 创业板指: 3723.52 (-3.10%)`) must be suppressed. All three wrapper scripts (`run_morning.py`, `run_noon.py`, `run_closing.py`) use the same pattern:

```python
import io
old_stdout = sys.stdout
sys.stdout = io.StringIO()       # suppress all stdout
# ... subprocess calls collect data and send cards ...
sys.stdout = old_stdout          # restore, produces zero output
```

**Card sender scripts (2026-07-13, re-verified no_agent 2026-07-13):**

| Script | Location | Purpose |
|--------|----------|---------|
| `send_feishu_cards.py` | `/opt/data/scripts/` | Shared card-sending utility |
| `send_morning_cards.py` | `/opt/data/scripts/` | Morning: 7 cards |
| `send_noon_cards.py` | `/opt/data/scripts/` | Noon: 3 cards |
| `send_closing_cards.py` | `/opt/data/scripts/` | Closing: 5 cards |
| `run_morning.py` | `/opt/data/scripts/` | Wrapper: collect_data + send_cards |
| `run_noon.py` | `/opt/data/scripts/` | Wrapper: collect_data + send_cards |
| `run_closing.py` | `/opt/data/scripts/` | Wrapper: collect_data + send_cards |

**Additional cron jobs** (beyond main 3-push):

| Name | Script | Schedule (CST) | Mode | Purpose |
|:----|:------|:--------------:|:----:|:--------|
| 🔐 微博凭证看门狗 | `weibo_watchdog.py` | 07:30 daily | no_agent | Check Weibo cookie validity; auto-generate QR on expiry |
| 💰 加仓信号监控 | `monitor_buy_signals.py` | 16:30 trading days | no_agent | Check 4 weak funds for buy signals (silent if none) |
| 📊 全量持仓监控 | `monitor_all_funds.py` | 16:30 trading days | no_agent | 6-dimension scan of all 15 funds (silent if no alert) |
| 📡 数据源验证 | `auto_validate_sources.py` | Sat 10:00 | no_agent | Weekly API availability report |
...
- `fact_check_kol_claims(text, quotes, sectors, overview, northbound)` — 2026-07-15新增: 验证KOL数值断言(涨跌幅/成交额/北向) against market data

**KOL分析策略** 详见 `references/2026-07-15-session-fixes.md`:
- 仅今日信号帖做解读, 历史帖仅做趋势参考
- 不显示互动数, 评论仅提取主任回复(低权重)
- 无快照时3级降级数据源: 快照→存档→腾讯API prev_close
- QQ消息合并: 按3800字符切分, 非按卡片切分
...
### Script References (2026-07-13 update -- added cron wrapper scripts + card senders)

| Script | Location | Purpose | Created/Updated |
|--------|----------|---------|:---------------:|
| `weibo_watchdog.py` | `/opt/data/scripts/` | Weibo credential auto-detection watchdog (no_agent cron) | **New 2026-07-13** |
| `weibo_login_direct.py` | `/opt/data/scripts/` | Weibo QR login (background subprocess for credentials) | **New 2026-07-13** |
| `send_feishu_cards.py` | `/opt/data/scripts/` | Shared Feishu card-sending utility (send_card, send_card_with_tables, send_card_with_button) | **New 2026-07-13** |
| `send_morning_cards.py` | `/opt/data/scripts/` | Morning: 7 cards via direct Feishu API | **New 2026-07-13** |
| `send_noon_cards.py` | `/opt/data/scripts/` | Noon: 3 cards via direct Feishu API | **New 2026-07-13** |
| `send_closing_cards.py` | `/opt/data/scripts/` | Closing: 5 cards via direct Feishu API (✅ verified 2026-07-13, 4/4 cards OK) | **New 2026-07-13** |
| `run_morning.py` | `/opt/data/scripts/` | Wrapper: collect_morning_data + send_morning_cards | **New 2026-07-13** |
| `run_noon.py` | `/opt/data/scripts/` | Wrapper: collect_noon_data + send_noon_cards | **New 2026-07-13** |
| `run_closing.py` | `/opt/data/scripts/` | Wrapper: closing_review + send_closing_cards | **New 2026-07-13** |
...
|----------|---------|---------|
| `eastmoney-api-verified-20260721.md` | `references/` | Eastmoney API实测验证记录，包含push2delay/kamt.kline等全部可用接口及字段映射。推荐新数据源前必须实测，见 common-pitfalls.md #14。
...
10. **Trend data must be visual, not raw text** (2026-07-04). Raw `date=value → date=value` format was rejected. Always use Unicode sparklines (`▁▂▃▄▅▆▇█`) for trend sequences. **Show `first→last` (not `min→max`)** plus `(振幅X.X%)`. Direction label must match `first→last`:
    - `+4.6% → -4.0% (振幅9.2%) 📉 跌幅扩大` ← correct (首→尾一致)
    - `-4.6% → +5.6% 📉 跌幅扩大` ← WRONG (min→max 与方向标签矛盾, fixed 2026-07-13)
    
    Sparkline algorithm: `spark_chars = ['▁','▂','▃','▄','▅','▆','▇','█']; ''.join(chars[min(int((v-min)/(max-min)*7),7)])`.

11. **Emoji convention: 按内容场景区分 (2026-07-13 用户确认，2026-07-13 最终定版)** — 用户要求「所有涨跌相关的我希望涨用红色emoji，跌用绿色emoji」，但随后纠正「基金表格用的 📈/📉 这个挺好，也可以用，你根据内容来用」。**不要一刀切全部改成🔴🟢**，保留基金表格的📈📉。最终分场景规则：

    | 场景 | 涨 | 跌 | 平 | 示例 |
    |:----|:---|:---|:---|:----|
    | 📊 大盘指数(沪深300/科创50...) | 🔴 | 🟢 | 🟡 | `🔴 +0.29%` `🟢 -1.79%` |
    | 🔥 板块热度/排行 | 🔴 | 🟢 | 🟡 | `🔴 +3.06%` `🟢 -6.68%` |
    | 🌊 北向资金 | 🔴 | 🟢 | 🟡 | `🔴 +10.2亿` `🟢 -40.38亿` |
    | 📈 分组趋势/操作评估 | 🔴 | 🟢 | 🟡 | `🔴 偏强` `🟢 跌幅扩大` |
    | 💰 **持仓基金涨跌** | 📈 | 📉 | ➖ | `📈 +1.23%` `📉 -5.36%` |
...
### 趋势方向标签正确逻辑 (2026-07-13 修复)

**不要展示 `min→max` + 方向标签。** 当中间值出现极端时显示范围与方向标签矛盾：
```
❌ 旧: -4.6% → +5.6% 📉 跌幅扩大  ← 从负到正，标签矛盾
  原因: min=-4.6%, max=+5.6%, first=-1.0%, last=-3.0%
✅ 新: +4.6% → -4.0% (振幅9.2%) 📉 跌幅扩大  ← 首→尾一致
```

**代码 (`closing_review.py` sparkline section):**
```python
first, last = vals[0], vals[-1]
week_min, week_max = min(vals), max(vals)
amplitude = week_max - week_min

if last > first and last < 0:     '📈 跌幅收窄'
elif last < first and last < 0:   '📉 跌幅扩大'
elif last > first and last > 0:   '📈 持续上行'
elif last < first and last > 0:   '📉 涨幅收窄'
elif first < 0 < last:            '🔄 由跌转升'
elif first > 0 > last:            '🔄 由升转跌'
else:                             '➖ 震荡'

f"| {group} | {spark} | {first:+.1f}% → {last:+.1f}% (振幅{amplitude:.1f}%) {direction} |"
```
...
### Self-Evolution System (v4-v7)

Full reference: `references/self-evolution-v4-v7.md` (loaded in this session).

Key tenets:
- **Verifiable recommendations** — decisions logged with `verification_3d: pending`, rechecked after 3 days.
- **Archived evolutions** — run `log_evolution.py` to snapshot system design to R2 after each change.
- **Weekly audit** — `system_health_audit.py` catches silent failures (JSONL quality, KOL resolution, delivery errors).
- **Auto-healing** — `deduplicate_archives.py` removes duplicate JSONL records daily.

### Platform Output Design Principle

**RESEARCH → DESIGN → IMPLEMENT** for any platform format task. Do not monkey-patch without understanding the target platform's message format capabilities first.

1. **Research** — What message types does the platform support? (text, post, card, etc.)
2. **Map** — What content needs to be delivered? (tables, bold text, links, images)
3. **Match** — Which message type(s) can render the required content correctly?
4. **Design** — Construct the message structure (JSON for card, tag:md for post, etc.)
5. **Implement** — Modify the adapter or output format
6. **Verify** — Send a test and confirm with the user

**Rule:** If you're writing a monkey-patch to change message formatting before you've confirmed the platform actually supports that format, you're doing it backwards. The user's exact words: "正常逻辑不应该是，在了解飞书机器人支持的消息格式类型及能力的基础上，结合我要推送展示的内容来设计消息结构形式吗".

**Skip the patch, research first.** This rule applies to ALL platform format tasks, not just Feishu.

### Feishu Message Format: CONFIRMED Findings (2026-07-13)

| Claim | Test Method | Result | Conclusion |
|-------|-------------|:------:|:----------:|
| `post`/`tag:md` supports GFM tables | Sent test with table via patched adapter | **❌ 还是普通文字形式** | 不渲染表格 |
| `interactive` card `tag:markdown` supports GFM tables | Sent card via Feishu API directly | **❌ 不是表格形式** | 卡片markdown也不渲染 |
| Card 2.0 native `table` component works | Sent card with `{"tag":"table"}` via Feishu API | **✅ 是表格** | **唯一可行方案** |
| sitecustomize.py import-hook patch works | Tested PYTHONPATH + auto-load | **✅** | 机制正常 |

**Known bug in sitecustomize.py patch:** `_make_patched_send()` fallback calls `original_send(self, chunk, ...)` where `original_send` is the original unpatched `send()`. But `self._build_outbound_payload` inside `original_send` is ALREADY the patched version, because `_build_outbound_payload` was patched BEFORE `send`. When the card fails and falls back, it re-enters the card path infinitely. **Fix:** pass `_strip_markdown_to_plain_text(chunk)` instead of `chunk` in the fallback. **Current workaround:** bypass adapter entirely — call Feishu API directly from send_feishu_cards.py.**

**⚠️ LLM agent's cron delivery does NOT go through sitecustomize card pipeline (2026-07-13):** The `sitecustomize.py` import-hook patches `FeishuAdapter.send()` at Python startup via `sys.modules` hook. But the cron agent's Python process does NOT import `sitecustomize.py` from `/opt/data/.feishu-deps/` (it's not in standard site-packages). This means the LLM agent's text response (from cron prompt) is delivered as **plain text, never a card**. **Fix:** set `no_agent=True` on all cron jobs. The wrapper script sends cards directly via Feishu API and suppresses stdout so cron delivery produces nothing. All three fund crons (morning/noon/closing) now use no_agent mode.

### Feishu Card Format: Phase-Based Implementation Pattern (2026-07-13)

When optimizing Feishu card format, use phased implementation:

1. **Design**: Identify specific component(s) to add (e.g. `hr`, `note`, `column_set`, `button`)
2. **Implement**: Modify `sitecustomize.py` `_build_card_from_content()` or `send_feishu_cards.py`
3. **Automate test**: Build card JSON from test content, verify structure (element count, tag types)
4. **Push test**: Send test cards via Feishu API directly
5. **Explain to user**: What files changed, what the structure will look like, what visual appearance to expect
6. **Get confirmation**: Wait for user's visual confirmation before proceeding

**Key rule**: Never batch all optimizations into one push. Each phase = one component type. See `references/feishu-card-optimization.md` for phase details and implementation patterns.

**⚠️ 大盘走势：column_set 不如原生 table（2026-07-13 验证）：** 尝试过用 `column_set`（左右双列）展示6列大盘走势数据。最终结论：`column_set` 左右行数不一致（左列2行/条、右列3行/条）→ 内容对不齐；压缩到1行/条 → 内容不匹配。最终方案：**去掉 `_is_quote_table()` 特殊处理**，6列表格走原生 `table` 组件，列对齐+表头行最清晰。`_build_quote_columns()` 函数保留未删，后续若需复用可重新激活。

When pushing tables to Feishu (早报/午报/收评), use this approach:

1. **Parse GFM tables** → use `_parse_table_block()` from sitecustomize.py
2. **Wrap in card** → use `_build_card_from_content()` from sitecustomize.py
3. **Split by section** → NEVER put all content in one card. Split by logical section:
   - Card 1: 外盘 + A股 指数 (blue)
   - Card 2: 量价分析 (indigo)
   - Card 3: 板块热度 (red)
   - Card 4: 持仓参考 (green)
   - Card 5: 操作参考 + 评估 (purple)
   - Card 6: KOL 分析 (wathet) — **含分析，非原文堆砌**
   - Card 7: 赛道新闻 (grey)

4. **Send via API** -- bypass adapter directly via `send_feishu_cards.py` helper:
```python
from send_feishu_cards import send_card, send_card_with_tables, send_card_with_button
# Pure markdown (no tables):
send_card("标题", "**markdown内容**", "blue")
# With native table parsing:
send_card_with_tables("标题", "| H1 | H2 |\n|---|---|\n| A | B |", "red")
# With link button (KOL cards):
send_card_with_button("KOL观点", "分析文本...", "🔗 查看原文", "https://weibo.com/u/xxx", "wathet")
# Supported template colors: blue, red, green, purple, wathet, grey, indigo, orange, yellow
```

See `references/feishu-message-format.md` for full details.
See `references/feishu-card-optimization.md` for Card 2.0 component analysis and optimization opportunities (column_set, hr, note, button).

### KOL Content: Must Include Analysis (2026-07-13)

**Never dump raw KOL posts.** The user explicitly asked why KOL content had no analysis. Every KOL section in the morning briefing must:

- **Extract core judgment** from each KOL's posts (e.g., "主任认为长鑫IPO前科技线有机会")
- **Note risk warnings** (e.g., "小浣熊预警AI泡沫可能破裂")
- **Give investment implications** (e.g., "科技/AI暂时不宜加仓")
- Quote the key post, then summarize — don't list every post in sequence

Three-line format for each KOL in the briefing:
```
**唐史主任司马迁**（近期主线：长鑫IPO影响 + 市场高波动）
- 🔑 **核心判断**: 长鑫完成缴款前市场持续高波，但整体力量向上
- ⚠️ **警示**: 业绩龙头爆亏的行业不要碰
- 📌 **对投资的意义**: 科技/AI目前震荡偏空，不宜加仓
```

### Content Splitting Strategy (2026-07-13)

When the data source (_morning_tables.md or _closing_tables.md) contains MULTIPLE tables, split into separate cards — one per logical section. Do NOT attempt to fit everything into one card, as the table parser may miss tables in very long content.

The correct approach: **one card per data category**, sent via direct Feishu API call, bypassing the adapter.
...
### Fund data cross-verification: gszzl vs ETF (2026-07-13 新增)

`fundgz.1234567.com.cn` API 在 **收盘后初期（15:00~16:00）** 返回的 `gszzl` 可能不稳定。2026-07-13 实例：华夏科创50ETF联接C 在16:01返回 `-5.36%`，但实际应为 `-3.42%`（与跟踪的科创50指数一致）。

在 `closing_review.py` `get_all_funds()` 调用之后，增加两层交叉验证：

**第一层：gszzl vs (gsz-dwjz)/dwjz 计算值**
- 计算理论涨跌: `(estimated_nav - nav) / nav * 100`
- 偏差 > 0.5% → 使用计算值替代 fundgz gszzl

**第二层：fundgz vs ETF/指数涨跌**
- ETF/指数跟踪映射: {'011613': ('科创50', 'quotes'), '013161': ('光伏', 'sectors'), ...}
- 偏差 > 1.5% → 用ETF数据替代（2026-07-13 实例就因此修正）
- 偏差 0.5~1.5% → 保留 fundgz 但标记
 
黄金联接基金(009478)始终为0.00%，独立用黄金ETF修正（沿用 2026-07-08 方案）。
...
### Data accuracy: cross-verify before every push (2026-07-13)

User mandate: "**所有数据你都要核查准确性**" — before any push, cross-check key data points:

1. **Fund gszzl vs ETF/index**: For ETF-linked funds, compare fundgz `gszzl` against the tracked ETF's market change. Discrepancy > 1.5% → use ETF data.
2. **gszzl vs computed**: Compare `gszzl` against `(estimated_nav - nav) / nav * 100`. Discrepancy > 0.5% → flag.
3. **Multiple time queries**: Data at 16:01 may differ from 16:30 (fundgz API transitions after close). If data seems wrong, re-query.
4. **Cross-platform verification**: If user reports discrepancy (e.g. 京东金融 -4.93% vs our -5.36%), check the actual source API and correct before pushing.
...
📌 LLM持仓约束→`references/llm-prompt-constraints.md`
📌 模型路由策略→`references/model-routing-investment.md`
📌 Cron交付格式→`references/cron-delivery-format.md`

- **cron_mode=deny blocks all cron jobs** — config.yaml has `approvals.cron_mode: deny` by default. Check with `hermes -p investment cron status`. Fix: `hermes -p investment config set approvals.cron_mode allow`. Also set `timezone: Asia/Shanghai`.
- **Weibo CLI broken on headless** — Use desktop API with QR-login cookies, not mobile API.
- **Fund API rate limiting** — Space requests > 1s apart; use 10s timeout.
- **Non-trading days** — Check `_*_skip.txt` files; skip if present.
- **R2 charset** — Always set `charset=utf-8` on markdown files for Chinese.
3. **Weibo credential expiry (~7 days)** — Re-login needed every ~7 days. Detect via `collect_morning_data.py` output: "❌ 微博凭据文件不存在" or sanity report showing "KOL: 0条". **Automated watchdog** runs daily at 07:30 CST (cron `eca4c57f4737`, no_agent script `scripts/weibo_watchdog.py`): checks credential validity; if expired, generates QR code and pushes to user chat via MEDIA: protocol. See `references/weibo-login-workflow.md` for manual re-login steps.
...
38. **Cron `last_run_at: null` = 作业已配置但调度器从未触发** — `cronjob(action="list")` 显示 `last_run_at: null` 意味着该 cron 从未被当前网关实例的调度器执行过。最常见的场景：作业在**当前网关启动之后**才被创建/更新 → 调度器载入时发现下次执行时间已过（如16:00创建但当前16:01）→ 跳过今天，明天正常触发。**诊断方法**：检查 `next_run_at` 是否为未来的正确时间；若正确，等下次触发即可。**立即修复**：`cronjob(action="run", job_id="...")` 手动触发验证。**2026-07-13 实例**：收盘复盘 cron `415fc1837c2b`，数据采集脚本跑了（`_closing_tables.md` 时间16:01）但 `last_run_at: null`，卡片推送从未执行。手动 `send_closing_cards.py` 后4张卡片全部✅\n\n39. **Cron script 路径必须是相对路径，解析到 `~/profiles/investment/scripts/`** — 2026-07-13 修复。三个 cron job 的 script 字段为 `run_morning.py`、`run_noon.py`、`run_closing.py`，解析到 `/opt/data/profiles/investment/scripts/`（空目录）。实际脚本在 `/opt/data/scripts/`。修复方法：在 profile scripts 目录创建软链接：`ln -sf /opt/data/scripts/run_*.py /opt/data/profiles/investment/scripts/`。**注意**：Hermes cron API 不接受绝对路径（rejected with "Script path must be relative to ~/.hermes/scripts/"），只能通过相对路径+软链接方式解决。\n\n40. **Markdown 表格必须有分隔行 `|---|---|` 才能被 sitecustomize 识别为原生 table** — 2026-07-13 修复。`collect_noon_data.py` 的量价分析表格缺少 `|:---|:----:|:----:|:--------:|` 分隔行，导致 `_build_card_from_content()` 当作普通文本处理。其它推送（早报 `collect_morning_data.py`、收盘 `closing_review.py`）的量价表格都有分隔行，没问题。所有手动构建的 markdown 表格必须包含分隔行，否则卡片表格不渲染。