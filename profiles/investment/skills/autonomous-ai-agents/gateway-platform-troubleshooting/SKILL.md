...
- **Gateway-native platforms (QQBot, Weixin) need `platforms.<name>.enabled: true` in the root config.yaml's `platforms:` section**, even though they're NOT plugin-based. They register directly in `gateway.py` and need the config block to start. Without it, `.env` credentials are silently ignored at runtime — the gateway process never calls the adapter.
- **Default profile = root Hermes home directory** in container deployments (e.g. `/opt/data/` mapped from `~/.hermes-main`). There is NO separate `profiles/default/` directory — the root `config.yaml` IS the default profile's config. Profile directories under `profiles/<name>/` are for secondary profiles only.
...
### Layer 1b: Check Root Config for Platform Entry

Even if `.env` has correct credentials, the platform won't start if the root config.yaml's `platforms:` section is missing the entry. This is the most commonly overlooked check for gateway-native platforms (QQBot, Weixin):

```bash
# Check if the platform has an entry in the platforms: section
grep -A 30 "^platforms:" /opt/data/config.yaml | grep -B 1 -A 1 "<platform_name>"

# For example, check for qqbot:
grep -A 30 "^platforms:" /opt/data/config.yaml | grep -B 1 -A 1 "qqbot"
```

**Expected output for each enabled platform:**
```yaml
  qqbot:
    enabled: true
```

**Pitfall:** QQBot and Weixin are **gateway-native platforms** (not plugin-based), so they only need the `platforms.xxx.enabled: true` entry — they do NOT need to be in `plugins.enabled`. Conversely, Feishu and DingTalk are **plugin platforms** — they need BOTH `plugins.enabled` AND their platform entry in config.yaml.
...
### Add PYTHONPATH to Gateway Run Script

The s6 supervised gateway reads its environment from `/run/service/gateway-default/run`. Add the `PYTHONPATH` to it:
...
**⚠️ Diagnostic first:** If QQ won't connect despite correct `.env` credentials, check the root config.yaml's `platforms:` section for a `qqbot: enabled: true` entry. Gateway-native platforms (QQBot, Weixin) are NOT controlled by `plugins.enabled` — they need their own `platforms.qqbot.enabled: true` config block. Without it, the gateway process never initializes the QQ Bot adapter regardless of what's in `.env`.

```bash
# Check if QQ platform is enabled in root config
grep -A 1 "^  qqbot:" /opt/data/config.yaml
# Expected: qqbot: enabled: true  (or just the key existing under platforms:)

# If missing, look for the platforms: section
grep -A 20 "^platforms:" /opt/data/config.yaml | head -20
# Add qqbot entry if not present
```

The fix if missing:
```yaml
# In the platforms: section of root config.yaml
  qqbot:
    enabled: true
```
...
**🐛 Known bug: `connect()` missing `is_reconnect` parameter:**

The QQ bot adapter's `connect()` method signature (`async def connect(self) -> bool:`) lacks the `is_reconnect` keyword argument that the gateway runner (`run.py`) passes when reconnecting. This causes `"QQAdapter.connect() got an unexpected keyword argument 'is_reconnect'"` in `gateway_state.json`, with QQ stuck in `state: retrying`.
...
print(f'state={qq.get(\"state\")} error={qq.get(\"error_message\")}')
"
```
Expected output: `state=retrying error=QQAdapter.connect() got an unexpected keyword argument 'is_reconnect'`

**Fix — patch the adapter code (requires root):**
```bash
...
The fix adds `*, is_reconnect: bool = False` to the signature and `del is_reconnect` (silences unused-argument linters) to match the `BasePlatformAdapter.connect()` contract that `run.py` expects. After fix, restart gateway: `docker exec hermes-main hermes gateway restart`.

The Python fix script at `/opt/data/fix_qqbot_adapter.py` automates both the patch and the gateway restart.
...
**Message formatting:** Feishu rich text (`post` + `tag:md`) does **NOT** render GFM tables — it displays `|` delimiters as raw text. The adapter's table-downgrade (to plain text) was intentional: "Feishu post-type 'md' elements do not render markdown tables". A monkey-patch at `sitecustomize.py` in `.feishu-deps/` can bypass the downgrade, sending tables as `post`/`md`, but this only improves non-table markdown elements (bold, headers) — tables themselves still show as raw `|` text. **For real table rendering, use Feishu Card messages** (`interactive` with `tag: markdown`). See `references/feishu-message-format.md` for the full analysis and card implementation notes.
...
### 🔥 Inside-Gateway Restart Dilemma

**Problem:** When running inside a Hermes session served by the gateway (e.g., in a Feishu/QQ Bot/Telegram conversation), you **cannot** use `kill` or `hermes gateway restart` — the terminal tool blocks it because killing the gateway would kill your own session. This produces:
```
...
**A — `delegate_task` (cleanest):** Spawn a subagent with its own terminal session:
```python
delegate_task(
    goal="Restart the gateway for profile X",
    context="Kill PID X (or use s6-svc -t /run/service/gateway-X/), wait for s6 to restart, verify new PID"
)
```

**B — nohup + disown script:** Write a small script that detaches from the process group:
```bash
cat > /tmp/gw_restart.sh << 'SCRIPT'
#!/bin/bash
sleep 3
...
nohup /tmp/gw_restart.sh < /dev/null > /tmp/gw_restart_out.log 2>&1 & disown
```

**C — docker exec (containerized deployment):** Access from outside the container:
```bash
docker exec <container> hermes gateway restart
# Or via s6:
docker exec <container> s6-svc -r /run/service/gateway-<name>/
```

**After restart:** The `--replace` flag lets the new gateway assume the old session. The current conversation resumes with a new gateway PID — the user experiences a brief interruption then `"Gateway restart completed"`.
...
- **`/opt/data/SOUL.md` being read-only (0444) breaks `hermes gateway setup`** — When running `gateway setup` to configure a new platform (or any CLI command that calls `save_env_value()`), the function `ensure_hermes_home()` → `_ensure_default_soul_md(home)` tries to write SOUL.md. If the file is `-r--r--r--` (0444), it fails with `PermissionError: [Errno 13] Permission denied: '/opt/data/SOUL.md'`. This shows up as a confusing Python traceback, not a helpful error message. **Fix: `chmod 644 /opt/data/SOUL.md`.**
