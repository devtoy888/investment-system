#!/usr/bin/env python3
"""
R2 Upload + Verification Tool

Upload a file to R2 with correct Content-Type, then automatically verify:
  - HTTP status 200
  - Content-Type header matches expectation
  - UTF-8 encoding validity
  - Chinese text readability (for Chinese-content files)
...
CONTENT_TYPE_MAP = {
    '.md': 'text/plain; charset=utf-8',    # text/plain for max browser compat
...
def verify_url(url, expected_ct=None, check_chinese=True, filepath=None):
    """Verify remote file: status, Content-Type, encoding, Chinese readability."""
    results = []
    ...
    try:
        ...
        return [{'check': 'HTTP 请求', 'status': '❌', 'detail': str(e)}]
    ...
    results.append({'check': '编码检测', 'status': '✅' if charset == 'utf-8' else '⚠️',
                    'detail': charset})

    if filepath:
        ext = os.path.splitext(filepath)[1].lower()
        keywords = CHINESE_KEYWORDS.get(ext, ['方案', '配置'])
...
def upload_and_verify(local_path, remote_key=None):
    """Upload file to R2 and verify it."""
    # Dynamic import to avoid circular dependency
    from r2_uploader import R2Uploader  # noqa
...
def verify_only(url):
    """Verify an existing R2 URL without uploading."""
    print(f'\n═══ 验证远程文件 ═══')
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print(f'用法:\n  python3 {sys.argv[0]} local.md [remote-key]\n'
              f'  python3 {sys.argv[0]} --verify-only <url>')
        sys.exit(1)
