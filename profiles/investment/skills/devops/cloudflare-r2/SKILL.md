...
### Content-Type for Chinese Markdown Files

When uploading markdown files with Chinese characters, the CORRECT pattern is:

```python
# CORRECT — preserves markdown semantics AND renders Chinese correctly:
r2.upload_file('doc.md', 'path/doc.md', content_type='text/markdown; charset=utf-8')
```

The key is `charset=utf-8` — without it, browsers default to Latin-1 and garble Chinese. `text/markdown` (not `text/plain`) tells browsers to render tables/headings/lists as formatted text.

**Pitfall: `upload_to_r2()` did not pass content_type to the subprocess.**
`fund_tools.upload_to_r2(file_path, key, content_type)` calls `r2_uploader.py` via `subprocess.run([python, script, file_path, key])`, silently dropping the third argument. Fixed 2026-07-15 by appending content_type to args when non-empty. Any R2 upload that needs a specific content_type must check this function passes it through.

### Long docs → companion HTML pages

For markdown documents >50 lines viewed on mobile, `text/markdown` renders as raw text (no formatting). **Create a companion `.html` page** using the `fetch` + `marked.js` pattern:

Pattern: create an HTML file that:
1. Fetches the `.md` file from R2 via `fetch()`
2. Renders it via `marked.parse()`
3. Applies dark-theme CSS with status badge coloring
4. Is mobile-responsive (viewport meta tag + responsive grid)

Reference: `references/markdown-html-rendering.md` for the full template.

```python
# Upload both:
uploader.upload_file('report.md', 'path/report.md', content_type='text/markdown; charset=utf-8')
uploader.upload_file('report.html', 'path/report.html', content_type='text/html; charset=utf-8')
```
...
|-----------|-------------|-------|
| .md (Chinese) | `text/markdown; charset=utf-8` | MUST set charset or Chinese garbles |
| .html (companion) | `text/html; charset=utf-8` | For mobile viewing of long docs |
| .json / .jsonl | `application/jsonl; charset=utf-8` or `application/json; charset=utf-8` | |
| .csv | `text/csv; charset=utf-8` | |
| Images | `image/png`, `image/jpeg`, etc. | |

### Auto-verify after upload

⚠️ **`/opt/data/scripts/r2_upload_and_verify.py` is BROKEN** (as of 2026-07-15): it does `from r2_uploader import R2Uploader` but `r2_uploader.py` lives at `/opt/data/r2_uploader.py` — NOT in `scripts/`, so the import fails with `ModuleNotFoundError`. Do NOT use it.

**WORKING invocation — call `r2_uploader.py` directly** (it has a `__main__` CLI entry):

```bash
# From inside the hermes-main Docker container:
cd /opt/data && \
  export $(grep -E '^R2_' /opt/data/profiles/investment/.env | xargs) && \
  PYTHONPATH=/opt/data python3 r2_uploader.py \
    /local/path/report.md \
    fund-system/reports/report.md \
    'text/markdown; charset=utf-8'

# Same for the companion HTML page:
PYTHONPATH=/opt/data python3 r2_uploader.py \
  /local/path/report.html \
  fund-system/reports/report.html \
  'text/html; charset=utf-8'
```

**CRITICAL CLI note:** the uploader's CLI is `python3 r2_uploader.py <file_path> <key> [content_type]` — there is **NO `upload` subcommand**. Passing `upload` as the first arg makes it treat `upload` as the file path → `FileNotFoundError: 'upload'`. Always pass file_path first.

**Credentials:** `R2Uploader()` reads `R2_ACCOUNT_ID / R2_BUCKET / R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY` from env. The container shell does NOT have them by default — `export $(grep -E '^R2_' /opt/data/profiles/investment/.env | xargs)` loads them (this does NOT modify `.env`, only exports into the shell). Never hardcode keys.

**Verify after upload** (separate curl, since the verify wrapper is broken):
```bash
curl -sI https://hermes-main-media.devtoy.xyz/fund-system/reports/report.md
# Check: HTTP/1.1 200  +  Content-Type: text/markdown; charset=utf-8
```
Or use `python3 -c "import urllib.request; r=urllib.request.urlopen(url); print(r.headers.get('Content-Type'), len(r.read()))"` to confirm UTF-8 Chinese readability.

Output includes the public URL on success (e.g. `https://hermes-main-media.devtoy.xyz/fund-system/reports/report.md`).
...
- **`r2_uploader.py` MUST have a `__main__` entry point** if it is called as a subprocess CLI (e.g., `subprocess.run([python, "r2_uploader.py", file_path, key])`). Without `if __name__ == "__main__":`, the class-based module runs zero code when invoked as a script — stdout is empty → caller receives None → upload silently fails. Discovery: fund_tools.upload_to_r2() called r2_uploader.py via subprocess for years but ALL uploads returned None. Fixed 2026-07-15 by adding CLI entry point.
