...
⚠️ **关键约束**: cron 的 `deliver=origin` 和 `deliver=qqbot:Home` 对 QQ Bot 均返回 500 错误（路径解析问题）。全部推送必须走**脚本内部** `send_markdown_in_chunks()`（位于 `/opt/data/scripts/send_qq_bot.py`，openid 硬编码）。

三大推送任务 (`d41a3db12dda`, `30b693051960`, `415fc1837c2b`) 统一配置：`deliver=local`, `no_agent=true`，由 shell 脚本调用 `send_markdown_in_chunks` 完成投递。

详见 `references/cron-qq-push.md`。
