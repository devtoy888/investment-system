...
Run `hermes setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](https://hermes-agent.nousresearch.com/docs/integrations/nous-portal).
...
## Update Behavior [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#update-behavior "Direct link to Update Behavior")
...
#### `terminal.home_mode` [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#terminalhome_mode "Direct link to terminalhome_mode")
...
#### Container lifecycle [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#container-lifecycle "Direct link to Container lifecycle")
...
#### Environment variable overrides [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#environment-variable-overrides "Direct link to Environment variable overrides")
...
### Write approval for skill writes [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#write-approval-for-skill-writes "Direct link to Write approval for skill writes")
...
When on, skill writes are staged under `~/.hermes/pending/skills/` and reviewed with `/skills pending`, `/skills diff <id>`, `/skills approve <id>`, `/skills reject <id>` — from the CLI or any messaging platform. Toggle at runtime with `/skills approval on|off`. Memory has the same gate (`memory.write_approval`, below). Full walkthrough: [Gating agent skill writes](https://hermes-agent.nousresearch.com/docs/user-guide/features/skills#gating-agent-skill-writes-skillswrite_approval).
...
With `memory.write_approval: true`, memory writes need your approval before they land: interactive CLI turns prompt inline; messaging sessions and the background self-improvement review stage the write for `/memory pending` → `/memory approve <id>` / `/memory reject <id>` review. Toggle at runtime with `/memory approval on|off`. See [Controlling memory writes](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory#controlling-memory-writes-write_approval).

## Context File Truncation [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#context-file-truncation "Direct link to Context File Truncation")
...
## Iteration Budget [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#iteration-budget "Direct link to Iteration Budget")
...
## Standing Goals (`/goal`) [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#standing-goals-goal "Direct link to standing-goals-goal")
...
`max_turns` caps how many continuation turns a goal can drive before Hermes auto-pauses it and asks the user to `/goal resume`. It protects against judge false negatives (goal actually done but judge says continue) and unbounded model spend on fuzzy or unachievable goals. See [Goals](https://hermes-agent.nousresearch.com/docs/user-guide/features/goals) for the full feature.
...
Tonbi's AI Garage24.7K subscribers
...
MoA is the one exception: reasoning depth for Mixture-of-Agents is configured **per slot** in the MoA preset (`moa.presets.<name>.reference_models[].reasoning_effort` / `aggregator.reasoning_effort`), not on the `moa_reference`/`moa_aggregator` auxiliary blocks — see [Mixture of Agents](https://hermes-agent.nousresearch.com/docs/user-guide/features/mixture-of-agents).
...
Context compression has its own `compression:` block for thresholds and an `auxiliary.compression:` block for model/provider settings — see [Context Compression](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#context-compression) above. The primary fallback chain uses a top-level `fallback_providers:` list — see [Fallback Providers](https://hermes-agent.nousresearch.com/docs/integrations/providers#fallback-providers). All three follow the same provider/model/base\_url pattern.

### Per-task fallback chain for auxiliary tasks [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#per-task-fallback-chain-for-auxiliary-tasks "Direct link to Per-task fallback chain for auxiliary tasks")
...
These models use _adaptive_ thinking and don't accept the usual `reasoning.effort`
...
#### Per-Model Reasoning Overrides [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#per-model-reasoning-overrides "Direct link to Per-Model Reasoning Overrides")
...
## Tool-Loop Guardrails [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#tool-loop-guardrails "Direct link to Tool-Loop Guardrails")
...
- **Write denied** — path is on the credential denylist or outside `HERMES_WRITE_SAFE_ROOT` (see [File write safety](https://hermes-agent.nousresearch.com/docs/user-guide/security#file-write-safety))
...
### Deny rules [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#deny-rules "Direct link to Deny rules")
...
Patterns are case-insensitive fnmatch globs and must be quoted in YAML (a bare leading `*` is a parse error). See [Security — User-Defined Deny Rules](https://hermes-agent.nousresearch.com/docs/user-guide/security#user-defined-deny-rules-approvalsdeny) for details.
...
## Network [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#network "Direct link to Network")
...
## Onboarding [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#onboarding "Direct link to Onboarding")
...
## Dashboard [​](https://hermes-agent.nousresearch.com/docs/user-guide/configuration\#dashboard "Direct link to Dashboard")
...
- `oauth` / `basic_auth` / `drain_auth` — auth provider config read by the bundled dashboard-auth plugins. The drain secret itself is **not** set here; it's provisioned via the `HERMES_DASHBOARD_DRAIN_SECRET` env var. See [Web Dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard) for full auth setup.
...
- [Update Behavior](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#update-behavior)
...
- [Skill Settings](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#skill-settings)
  ...
  - [Write approval for skill writes](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#write-approval-for-skill-writes)
- [Memory Configuration](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#memory-configuration)
- [Context File Truncation](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#context-file-truncation)
...
- [Iteration Budget](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#iteration-budget)
- [Standing Goals (`/goal`)](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#standing-goals-goal)
...
- [Auxiliary Models](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#auxiliary-models)
  ...
  - [Per-task fallback chain for auxiliary tasks](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#per-task-fallback-chain-for-auxiliary-tasks)
...
- [Tool-Loop Guardrails](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#tool-loop-guardrails)
...
- [Smart Approvals](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#smart-approvals)
  - [Deny rules](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#deny-rules)
...
- [Network](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#network)
- [Onboarding](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#onboarding)
- [Dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/configuration#dashboard)