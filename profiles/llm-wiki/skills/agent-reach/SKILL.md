...
- **小红书** — after starting xiaohongshu-mcp binary, must register with `mcporter config add xiaohongshu http://localhost:18060/mcp`. On headless ARM64 servers, xiaohongshu-mcp needs Chromium which is unavailable — use OpenCLI on desktop instead. mcporter config saves to /tmp by default; write to /opt/data/config/mcporter.json for persistence.
