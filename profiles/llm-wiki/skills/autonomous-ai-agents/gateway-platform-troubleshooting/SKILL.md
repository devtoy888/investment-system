...
- `references/feishu-card-rendering.md` — Feishu Card 2.0 消息渲染修复：两个 Markdown 渲染器根因分析（`post+tag:"md"` vs `interactive+tag:"markdown"`），代码定位（`_build_outbound_payload()` 行 4470），修复方法，权限说明，上游 PR/issue 引用
