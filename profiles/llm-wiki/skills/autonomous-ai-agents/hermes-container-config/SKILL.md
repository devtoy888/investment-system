...
**For Feishu specifically**, the setup per profile is standard:
```yaml
# Profile A's config.yaml (under gateway.platforms.feishu)
feishu:
  app_id: cli_xxxxx
...
Each profile uses `hermes gateway setup` independently to register its Feishu bot credentials. With multiplex mode, all profiles' Feishu bots connect through the single gateway process. See `references/feishu-multi-bot-architecture.md` for a worked example.
...
| Action | Standard | Docker/s6 |
|--------|----------|-----------|
| Edit config.yaml | `hermes config set key val` | `sed -i` via terminal |
...
## Understanding the Multi-Model Architecture

Hermes has **four distinct model layers**. When switching providers, you may need to change multiple layers:

| Layer | Config path | What it does | Typical choice |
|-------|------------|-------------|----------------|
| **Primary** | `model.default` + `model.provider` | All chat/conversation | Best capability/cost balance |
| **Delegation** | `delegation.model` + `delegation.provider` | Sub-agents (parallel tasks) | Same or heavier than primary |
| **Fallback chain** | `fallback_providers[]` | Automatic failover when primary is down | 2-3 free + 1 paid as last resort |
| **Auxiliary** | `auxiliary.*.*` | Vision, compression, approval, title gen, curator | **Cheapest/free** — don't waste paid quota here |

### Common Architecture Patterns

**All-in-one (same model everywhere):**
```
Primary=MiMo → Delegation=MiMo → Fallback=DeepSeek → Auxiliary=OpenRouter free
```
Best for: cost efficiency, consistent behavior.

**Split (primary ≠ delegation):**
```
Primary=MiMo-v2.5 → Delegation=DeepSeek-v4-Pro → Fallback=... → Auxiliary=OpenRouter free
```
Best for: keeping heavy sub-agent work on a proven model while main chat uses cheaper plan.

### When to Change Each Layer

- **Primary only**: Quick switch for testing. Sub-agents and fallback still use old model.
- **Primary + Delegation**: Full migration. All active processing uses new model.
- **All four layers**: Rare — only needed when switching credential providers entirely.

### 🔑 Key Insight: Auxiliary Should NOT Burn Quota

Auxiliary tasks (compression, summarization, approval, curation, title generation) run **many times per session** and consume significant tokens. **Never route auxiliary tasks through a quota-limited provider** (Token Plan, paid API) — use free OpenRouter models instead:
...
auxiliary:
  ...
  compression:
    provider: openrouter
    model: cohere/north-mini-code:free
  # ... repeat for approval, curator, skills_hub, title_generation
```
...
echo 'XIAOMI_API_KEY=***' >> /opt/data/.env
...
**For MiMo Token Plan specifically**, you need TWO env vars:
```
XIAOMI_API_KEY=tp-xxxxx
XIAOMI_BASE_URL=https://token-plan-cn.xiaomimimo.com/v1
```
(Singapore cluster: `https://token-plan-sgp.xiaomimimo.com/v1`, Europe: `https://token-plan-ams.xiaomimimo.com/v1`)

### 2. Update config.yaml (Primary + Delegation + Fallback)

When switching models, MOST changes need multiple layers:

```bash
# ── Change PRIMARY model ──
hermes config set model.default new-model-name
hermes config set model.provider new-provider
hermes config set model.base_url ""           # Empty = provider default URL

# ── Change DELEGATION model (separate step!) ──
hermes config set delegation.model new-model-name
hermes config set delegation.provider new-provider

# ── Change FALLBACK entry (if provider changes) ──
hermes config set fallback_providers.3.model deepseek-v4-flash
hermes config set fallback_providers.3.provider deepseek
```

**Common mistake #1:** Changing only `model.*` without `delegation.*`. Sub-agents keep using the old model, causing confusing behavior.

**Common mistake #2:** Changing fallback entries via OpenRouter when the provider is now direct. "OpenRouter deepseek" (openrouter provider) ≠ "direct deepseek" (deepseek provider).

**For MiMo setup (complete example):**
```bash
...
```bash
echo "=== Primary ===" && grep -A4 "^model:" config.yaml
echo "=== Delegation ===" && grep -A4 "^delegation:" config.yaml
echo "=== Fallback link 3 ===" && grep -A2 "'3':" config.yaml
echo "=== Env vars ===" && grep -n "^XIAOMI\|^DEEPSEEK\|^OPENROUTER" .env
```

### 3. Test Model Connectivity (Before Restart)

Use `hermes chat -q` to verify the model works with the new config:

```bash
export HERMES_HOME=/opt/data/profiles/<profile>
/opt/hermes/bin/hermes chat -q "回复OK确认连通，请用中文回答" --quiet
```
Expected: model responds in Chinese with "OK，连通确认。"

If it fails:
- Check env vars are set: `grep "^XIAOMI" .env | od -c`
- Verify provider name spelling: `xiaomi` (NOT `mimo`, NOT `xiaomi-mimo`)
- Verify model name: `mimo-v2.5` (NOT `mimo-v2.5-lite`)
- Check XIAOMI_BASE_URL is the Token Plan endpoint (not the pay-as-you-go one)
...
# Structure B: Custom provider definition (named, reusable)
providers:
  custom:
    agnes:
      base_url: https://apihub.agnes-ai.com/v1
      api_key: sk-inline-key        # inline OR key_env
      # key_env: CUSTOM_AGNES_API_KEY  # alternative: read from env var
      model: agnes-2.0-flash

# Structure C: Legacy custom_providers (list format)
custom_providers:
  - name: agnes
    base_url: https://apihub.agnes-ai.com/v1
    api_key: sk-xxx
...
**Common mistake:** A provider defined in `providers.custom.agnes` is NOT the primary model — it's only used when explicitly referenced (e.g. via `custom:agnes` in fallback chain or cron job `--model`). The primary model is always `model.default` + `model.provider`.
...
- See `references/agnes-api-test-results.md` for detailed test results.
