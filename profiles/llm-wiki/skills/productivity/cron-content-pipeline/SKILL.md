...
- **🎯 User convention: all times are Beijing time (CST/UTC+8).** This user communicates all cron schedules in Beijing time (CST, UTC+8) unless the timezone is explicitly stated. When the user says "凌晨 2 点" → 02:00 CST = `0 18 * * *` UTC. When reporting next_run_at or communicating schedules back → always convert to Beijing time first. Exception: if the user says "UTC 时间" or specifies a timezone, use that. This convention applies to ALL cron-related interactions with this user — stored in memory.
- **Cron runs in UTC internally.** The cronjob `schedule` field accepts cron expressions in UTC. `0 0 * * *` = 08:00 CST. `0 8 * * *` = 16:00 CST. Always convert user's Beijing time to UTC before storing, and always check `next_run_at` after any change.
