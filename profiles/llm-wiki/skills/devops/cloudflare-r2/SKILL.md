...
references:
  ...
  - beautiful-md-report.md — **Custom Python renderer** for user-facing changelog/repair reports: styles EVERY markdown element (hero, tables, badges, lists, bold, inline code, checkboxes), dark/light adaptive, with verification steps. Use when the user will actually read/share the HTML.
scripts:
...
**Host-side scripts**: When running R2 scripts outside the Docker container (e.g., from Termius or host crontab), the shell won't have R2 env vars. Use the `load_env()` pattern to auto-read from profile `.env` files (enumerates ALL profiles, returns on first existing match — never hardcode only the main profile):

```python
import os, sys, glob
sys.path.insert(0, '/opt/data')
def load_env():
    # Docker: /opt/data/.env (main) + /opt/data/profiles/<name>/.env
    # Host : ~/.hermes-main/.env   + ~/.hermes-main/profiles/<name>/.env
    ...
    for path in paths:
        if os.path.exists(path):
            ...
            return  # hit one profile → return (avoid cross-profile bleed)
load_env()
from r2_uploader import R2Uploader
uploader = R2Uploader()
```
...
## Upload Pattern
## Pitalls
- **CRITICAL: R2 object keys MUST be ASCII/English — never Chinese**
  Raw Chinese characters in an object key produce an UN-encoded URL (e.g. `https://bucket.devtoy.xyz/references/展示框架评估.md`). Clients (QQ, Feishu, browsers, copy-paste) insert spaces/newlines between `/references/` and the Chinese glyphs, **breaking the link**. The file IS uploaded and opens fine if pasted correctly — the link is just not directly clickable.
  - **Fix (done once, never repeat)**: always use an English/romanized key, e.g. `references/wiki-frontend-framework-eval.md`. Keep the LOCAL Wiki filename Chinese per SCHEMA; only the R2 *key* must be English. Decouple local name ↔ R2 key.
  - When sharing an R2 URL, use the percent-encoded form (`/references/%E5%B1...`) or the English key — never the raw-Chinese URL.
  - Verification: before upload, assert `key.isascii()`; if not, replace with an English slug.
- **CRITICAL: cross-profile .env reads are WRONG (2026-07-18)**
  Each Hermes profile's R2 credentials live in `~/.hermes-main/profiles/<name>/.env` (in-container: `/opt/data/profiles/<name>/.env`), NOT the main profile's `~/.hermes-main/.env`. The manual `load_env()` snippet below enumerates ALL profiles via `glob` and returns on the FIRST existing match — never hardcode only the main profile path (that reads the wrong profile's credentials; it only "works" if buckets coincidentally match).
  - Docker mapping: host `~/.hermes-main` → container `/opt/data`. Main profile `/opt/data/.env`; llm-wiki `/opt/data/profiles/llm-wiki/.env`; others `/opt/data/profiles/<name>/.env`.
  - Detect current profile: when running inside container `/opt/data`, prefer `/opt/data/profiles/<current-profile>/.env`. If unsure, enumerate all and log the matched path.
...
### ⚠️ Beautified HTML reports: render EVERY element type

When you generate an HTML report from Markdown (changelog, repair log, status page) for the user to read/share, do **not** stop at tables + hero. A report that styles tables but leaves paragraphs, lists, bold, and inline code as raw unstyled text is **worse than plain text** — the inconsistency looks broken and the user will have to point it out. Style **all** element types: headings, paragraphs, **bold**, `inline code`, ordered/unordered lists, task-list checkboxes, blockquotes, tables, horizontal rules. See `references/beautiful-md-report.md` for a complete custom Python renderer pattern (hero + stat cards, status badges, dark/light adaptive, verify-before-done checklist). After uploading, verify the *deployed* file with a cache-busting URL (`?nocache=N`) — R2/CDN caching can serve a stale old version.
...
- **R2 object key 必须英文/拼音，禁用中文**：`R2Uploader` 返回的 `public_url` 是**未百分号编码的原始 URL**（含裸中文）。客户端（QQ/飞书/浏览器）常在 `/` 与中文字符间插入空格/换行 → 链接断裂、无法点击。根治：上传 key 一律用英文短名（如 `references/wiki-frontend-framework-eval.md`），本地 Wiki 文件名仍按 SCHEMA 用中文，二者解耦。若已用中文 key，重传为英文 key + 删旧中文 key。
- **MD 与 HTML 双版内容必须对齐**：当用户要求"写 MD + 对应自适应 HTML 并上传 R2"时，两版须包含**相同章节与结论**（尤其新增的对比维度、问答、相关页面链接）。常见失误：MD 有"相关页面"链接列表而 HTML 只留标题无内容，或某框架评估仅在 MD 补、HTML 漏补。每次改动后**两版同步改、同步上传、同步验证**。
- **凭据读取必须按当前 profile，禁止跨 profile**：R2 凭据在 `.env`。各 profile 路径隔离——主 profile `~/.hermes-main/.env`（容器内 `/opt/data/.env`），llm-wiki profile `~/.hermes-main/profiles/llm-wiki/.env`（容器内 `/opt/data/profiles/llm-wiki/.env`）。host-side `load_env()` 模板默认读主 profile，在子 profile 会话中**必须改为读当前 profile 的 `.env`**，否则属跨 profile 读取（即便 bucket 共享、上传"碰巧"成功，也不合规）。当前进程运行于容器内 `/opt/data`，直接读 `/opt/data/profiles/<profile>/.env`。
- **上传后必须验证 Content-Type + 中文可读性**：用 boto3 `get_object` 直连校验（避开公网域名 403/CDN 缓存），确认 `Content-Type` 含 `charset=utf-8` 且关键中文关键词存在，再向用户交付链接。
