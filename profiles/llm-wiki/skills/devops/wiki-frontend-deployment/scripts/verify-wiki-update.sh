...
if [ -f "$SVG_FILE" ]; then
    echo "[1/5] SVG host file check: $SVG_FILE"
    ...
    echo "[1/5] ❌ SVG file not found: $SVG_FILE"
    errors=$((errors+1))
fi
echo ""
...
if [ -f "$PAGE_FILE" ]; then
    echo "[2/5] Page structure check: $PAGE_FILE"
    ...
    echo "[2/5] ❌ Page file not found: $PAGE_FILE"
    errors=$((errors+1))
fi
echo ""
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^$CONTAINER\$"; then
    echo "[3/5] Container sync check: $CONTAINER"
...
if [ -f "$BUILD_SCRIPT" ]; then
    echo "[4/5] Build script check: $BUILD_SCRIPT"
    ...
    echo "[4/5] ⚠️ No build script found — skip"
fi

echo ""

# 5. Docker log check for MkDocs build warnings
CONTAINER="llm-wiki"
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "^$CONTAINER\$"; then
    echo "[5/5] MkDocs build log check: $CONTAINER"
    LOGS=$(docker logs "$CONTAINER" --tail 30 2>&1 || true)
    
    UNRECOGNIZED=$(echo "$LOGS" | grep -c "unrecognized relative link" || true)
    ABSOLUTE=$(echo "$LOGS" | grep -c "absolute link" || true)
    OBSIDIAN_ERR=$(echo "$LOGS" | grep -c "ObsidianBridge.*No candidates" || true)
    
    if [ "$UNRECOGNIZED" -gt 0 ]; then
        echo "  ❌ $UNRECOGNIZED 'unrecognized relative link' warnings — links are broken"
        errors=$((errors+1))
    else
        echo "  ✅ No 'unrecognized relative link' warnings"
    fi
    
    if [ "$ABSOLUTE" -gt 0 ]; then
        echo "  ⚠️ $ABSOLUTE 'absolute link' INFO (harmless, optional fix)"
    else
        echo "  ✅ No 'absolute link' INFO"
    fi
    
    if [ "$OBSIDIAN_ERR" -gt 0 ]; then
        echo "  ❌ $OBSIDIAN_ERR ObsidianBridge errors — wikilinks unresolved"
        errors=$((errors+1))
    else
        echo "  ✅ No ObsidianBridge errors"
    fi
else
    echo "[5/5] ⚠️ Container $CONTAINER not running — skip"
fi
